...
namespace Infobip.Api.SDK.Tests.WhatsApp
{
    public class SendWhatsAppDocumentMessageTests : IClassFixture<MockedHttpClientFixture>
    {
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_FromInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            ...
            var request = new WhatsAppDocumentMessageRequest("", "447860099300",
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_ToInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            var responseMessage = GetResponseMessage();
            var apiClient = new InfobipApiClient(_clientFixture.GetClient(responseMessage));
            ...
            var content = new WhatsAppDocumentContent("http://example.com/media");
            var request = new WhatsAppDocumentMessageRequest("447860099299", "",
                Guid.NewGuid().ToString(),
                content);
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_MessageIdInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            var responseMessage = GetResponseMessage();
            var apiClient = new InfobipApiClient(_clientFixture.GetClient(responseMessage));
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_CallbackDataInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            var responseMessage = GetResponseMessage();
            var apiClient = new InfobipApiClient(_clientFixture.GetClient(responseMessage));
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_NotifyUrlInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            var responseMessage = GetResponseMessage();
            var apiClient = new InfobipApiClient(_clientFixture.GetClient(responseMessage));
        ...
        [Fact]
        public async Task SendWhatsAppDocumentMessage_Call_With_ContentInvalidInRequest_Throws_InfobipRequestNotValidException()
        {
            // Arrange
            var responseMessage = GetResponseMessage();
            var apiClient = new InfobipApiClient(_clientFixture.GetClient(responseMessage));

            var content = new WhatsAppDocumentContent("",
                new string('x', 3001),
                new string('x', 241));
            var request = new WhatsAppDocumentMessageRequest("447860099299", "447860099300",
                Guid.NewGuid().ToString(),
            ...
            exception.ValidationResults.Should().HaveCountGreaterThan(0);
            var errors = exception.ValidationResults.SelectMany(result => result.MemberNames.Select(s => s)).ToArray();
            errors.Should().Contain($"{nameof(request.Content)}.{nameof(WhatsAppDocumentContent.MediaUrl)}");
            errors.Should().Contain($"{nameof(request.Content)}.{nameof(WhatsAppDocumentContent.Caption)}");
            errors.Should().Contain($"{nameof(request.Content)}.{nameof(WhatsAppDocumentContent.Filename)}");
        }
